...
const Move = ({move}) => {
    ...
    function searchList(name) {
        ...
        const filteredList = state.moves.filter(move => move.name.includes(name) || move.type.includes(name.toLowerCase()));
        ...
        return (<ListItem containerStyle={[styles.cardStyle, {backgroundColor: state.theme.colors.card}]}>
                <ListItem.Content style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
                    ...
                    <View style={{width: 100, flexDirection: 'column'}}>
                        <Text style={{
                            fontSize: 20,
                            color: state.theme.dark ? "rgba(255," + (Math.floor(255 - (255 * (move.power / 120)))) + "," + (Math.floor(255 - (255 * (move.power / 120)))) + ",1)" : "rgba(" + (Math.floor((move.power / 100) * 255)) + ",0,0,1)"
                        }}>
